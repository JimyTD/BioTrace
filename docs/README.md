...
> 只留四类东西：**来时路**（当初为什么这样定）、**功能真源**、**运维真源**、**专题手册**。  
> 编号只属于当初的思考步骤（`planning/`）；实现侧文档一律用名字。  
> 更新日期：2026-08-10
...
| 加套册 / 加皮肤 / 出安卓包 | [`features/`](./features/) |
| 搞清「当初为什么这样定 / 为什么不那样做」 | [`planning/`](./planning/) |

**一件事只认一个真源**：功能状态以 `SPEC.md` 为准，线上现状以 `OPS.md` 为准。`planning/` 与 `archive/` 已冻结，**不要**当实现清单。**变更历史看 git log**，文档里不再手抄一份。
...
| 文件 | 内容 |
|------|------|
| [`planning/00-讨论进程与决策.md`](./planning/00-讨论进程与决策.md) | 讨论顺序、项目定位、三项关键约束、第 1 步结论与 Seek 实测 |
