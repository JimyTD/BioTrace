...
- 插件：`@capacitor/camera`（拍照）、`@capawesome/capacitor-file-picker`（相册原图）、`@capacitor/filesystem` + `@capacitor-community/file-opener`（应用内下载 APK 并唤起安装）
...
- [ ] 「从相册选择」打开系统相册（原图 FilePicker）；可多选
- [ ] 「拍照」可拍摄并上传
