...
- 底栏：`--nav-bg`、`--nav-active-bg`、`--nav-active-ink`（激活态勿复用冷绿 soft）、`--nav-texture`（可选纸纹，默认 `none`）
...
文件名与分层见 [`套册美术分层.md`](./套册美术分层.md)。加 `tide` 时复制同名文件到 `volumes/tide/` 即可换皮，**不必改** volumes 引擎或页面分支。

## 2.2 壳资源包（可选）

目录：`apps/web/public/shell/<themeId>/`。  
当前只有可选 `nav-texture.png`（顶栏/底栏纸纹）。没有文件时 `--nav-texture: none`，骨架照常。路径约定见 [`shellAssets.ts`](../../apps/web/src/themes/shellAssets.ts)、[`public/shell/README.md`](../../apps/web/public/shell/README.md)。不要把邮戳或 Tab 图标画进组件。
