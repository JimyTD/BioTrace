...
> 创建：2026-09-07；**2026-09-08 整理为移交版**（决策表 D1–D6b 齐备、各阶段完成线 §8、落点速查表 §9），接手 agent 从 §0 读完即可开工，无需原会话上下文。
...
- [x] ⑤`domesticated` 题答案直供；`SCALE_ITEM_KEYS` 不动。
- [x] ⑥引入警报 dom=true 不触发。
...
| [rarity/scale.ts](../../apps/api/src/rarity/scale.ts) | 判卷调度 | 二 | 键升 (taxonKey, dom)；`|dom` 分键不升版；⑤答案直供注入 |
| [rarity/scale-rubric.ts](../../apps/api/src/rarity/scale-rubric.ts) | 题定义/分批/打分 | 二 | `scaleBatchesForModel` 摘掉 domesticated；题定义不动 |
