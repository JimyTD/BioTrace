...
§4.2（2026-09-09）已拍板：翻 R5b / R7。只拍宠物时「已收录」和树都要认这一种。R2、R6 同日确认：宠物卡挂驯化卷稀有度 +「驯养物种」标签；套册照常点（宠物也是动物）。R6b 同日改口：品种库只纠偏，品种不详格只收空+排除词，其余非空另开格。格上文案「品种不详」。**R3 品种格呈现（全库预印灰格）未收口，见 §7。** 其余 R1 / R4 仍按 §4.1。判卷层六件套已定；缓存**不升 scale3**。
...
识图 prompt 新增字段：`domesticated: true|false|null`（null=不确定，按 D3 折到 wild）与 `breed_zh: string|null`（能看出品种就写名字，认不出或颜色俗名/串串/物种名为空）。填写规则照 D3 阶梯；后端三刀见 §4.3。**旧行不回刷**（既有原则，软档同例）；宠物类历史观察走后台「重算开包结算」通道逐条重跑。
...
- **R2 卡与稀有度**：（2026-09-09 翻）观察走驯化卷 12 题；宠物卡**挂**开包那档，聚合只升不降。另挂「驯养物种」名录同款标签。狼 UR 仍只在野生卡上。
- **R3 品种格呈现**：（未收口，§7）全库预印灰格、只亮自己那格——已否决，猫狗马都不许这么做。禁止按物种特殊处理，禁止用 `listed` 把对照表分成「能上卡 / 只能纠偏」。当前代码权宜：一律只显示已点亮的格子。以后另想呈现，不是把灰格加回来。
...
- **R5 现有图鉴侧（wild 侧）守恒**：dom=true 观察**不写入** `collectionEntries`（避免狼狗同卡）；确认为狼(wild)才点狼卡。宠物卡仍要在「已收录」里找得到——见 §4.2，不是把狗写回野生表。
- **R5b 树上口径**：（2026-09-09 翻）宠物条目挂到种级节点，不新开家犬枝。只养狗 → 犬科/犬属/`Canis lupus` 从收集长出，种级标签「家犬」。狼+狗 → 仍一个节点，详情两张卡。
- **R6 套册不牵连**：（2026-09-09 确认）套册照常点亮。宠物也是动物；家犬仍点犬科等槽。狼和狗点同一格，不并出两枚。代码本就如此，无需改。
- **R6b 品种格**：（2026-09-09 改口）库只纠偏，不是许可清单。品种不详格留下，但不再当「表里没有」的垃圾桶。格上文案「品种不详」。详见 §4.3。
- **R7 统计口径**：（2026-09-09 翻）首页「已收录 N」= 野生卡 + 宠物卡张数之和。宠物行留下当品种册入口。

引入警报：`dom=true` 不触发（§7）。题面不改学名。

### 4.2 方案：只拍宠物时，「已收录」和树不该像没遇见（2026-09-09，已拍板）

**现状**

从没拍过狼、新拍一条狗且 `domesticated=true`：

| 入口 | 现在 | 用户观感 |
|------|------|----------|
| 已收录 | 空（狗不进 `collectionEntries`） | 收下的东西找不到 |
| 已收录 N | 0 | 像什么都没收录 |
| 宠物 | 有「家犬」卡 | 唯一藏狗的地方 |
| 物种树 | 骨架（界→目）人人都有；科属种只从野生图鉴长。狗不上树 | 不是空白世界，但是**你的狗完全不在树上**；末端按 `got=0` 当未点亮 |
| 套册 | 家犬仍点犬科等槽（R6） | 和树、已收录口径打架 |

所以：已收录空着不合理。树也不是「空的」，是**预先填入的骨架还在，属于你的种枝没有**。

**建议（改 R7、R5b；不推翻 D2 / D5）**

驯养仍然**不在种下新开节点**（树还是 `Canis lupus` 这一种，不出现「家犬亚种」）。但驯养必须出现在「我收集过」的入口里。

1. **已收录列表混排两张卡。** 野生卡 + 宠物卡都出现。只养狗 → 能搜到「家犬」。又有狼又有狗 → 两行（狼、家犬），正是 D2 的「同键不同位拆卡」。点家犬进宠物卡（品种格）；点狼进野生卡（稀有度徽章）。**不要**把狗写回 `collectionEntries`，否则又变一张狼卡。
2. **「已收录 N」= 两表张数之和。** 只养狗则 N≥1。宠物那一行留下，当品种册入口，不是唯一藏狗的抽屉。
3. **树：宠物条目也挂到种级节点。** 不新增节点。只养狗 → 犬科 / 犬属 / `Canis lupus` 从收集长出来，种级标签用宠物俗名「家犬」，不要写「狼」。狼和狗都有 → 仍一个节点；详情里两张卡（野生 + 宠物）。这是一种两张收集卡，不是种下拆亚种。
4. **套册不动（R6）。** 改完之后树和套册都认这一种，口径对齐。

**不采用**：把狗当野生重新写入已收录（狼狗同卡，D5 作废）；树上为家犬另开节点（种下分叉，D5 作废）；只改空态文案、列表仍然没有狗（找不到的问题还在）。

**锚**：树上仍是生物种级树（现有 `treeModel` 骨架 + 收集生长），家犬像标本钉在 `Canis lupus` 上，标签写家犬。已收录像目录：每种身份一张卡。

已落地：`CollectionSpeciesPage` 混排两表；图鉴首页「已收录 N」两表之和；`CollectionTreePage` 建树并入宠物条目；种级展示名按来源选家犬/狼。表结构未动。宠物图鉴行仍在。

### 4.3 品种格怎么亮（R6b，2026-09-09 拍板）

品种库是对照表：把已知写法收到同一格。不在库里的名字不去鉴定是不是真品种，也不写进全局 JSON。

拍驯养动物后，看 `breed_zh` 三刀：

1. **对上库**（标准中文 / 英文 / 别名；去掉末尾「猫犬鸡牛马…」；**唯一包含**才认，如「英国短毛」→英国短毛猫。「短毛」能罩多个则不强行猜）→ 标准格。
2. **空，或排除表**（狸花/橘猫/三花/串串/混血/家猫家犬家牛/田园猫/黄牛奶牛…，见 `discard.json`）→ **品种不详**。
3. **其余非空** → 用模型写下的字**新开一格**（荷斯坦在扩库前、奥特曼、库外新名字）。只写在该用户这张卡的 `litFreeBreedsJson`。以后人手补进库，重算并进标准格。

不做编辑距离（金渐层 / 银渐层差一字不互认）。犬库「中华田园犬」仍吃田园犬/土狗；猫「田园猫」进品种不详（排除表，且量词不同，不会把田园犬误伤进去）。

扩库：`dog/cat/chicken/cow/horse.json`，Wikidata 品种实体 + 畜禽遗传资源保护名录摘录。库只纠偏。猫狗马牛鸡同一条展示路径，无按物种特殊处理。卡上呈现见 R3 / §7（当前权宜只出已点亮）。JSON 里残留的 `listed` 不参与展示。

> 结构性风险提示：树/图鉴/套册对「同一物种不同稀有度」的既有约定是**演化的自由**（观察级独立+卡级只升不降），不是**个体的并列**（同键并列两档）。A' v1 的拆卡把「演化」强行扳成「并列」，这才是它伤树严肃性的机制根源。

## 5. 品种普及度标记（认证口径已收口，见 §4.3）

- 品种库 `apps/api/data/breeds/`：cat / dog / chicken / cow / horse + `discard.json`。
- 普及度三档只做格子视觉区分，不叫稀有度、不进档位（§3 已论证）。自由格无普及度，不描边。
- 展示词需过术语表（`messages-glossary.mdc` 铁律）。格上标准名来自库，自由格用模型原文。
- 认证三刀见 §4.3。颜色俗名走排除表，不进库。
...
2. **阶段二 · 判卷六件套**：§3.1 全表（`domesticated` 题答案直供；题面不改学名；缓存分 `|dom` 不升版）+ 锚点表键折叠同形。R2：dom 卷照判、宠物卡挂开包档 + 驯养物种标签。
3. **阶段三 · 宠物图鉴子系统**：新表与分轨入口（dom=true 不进现有 collectionEntries）、wild 侧守恒、展示 tags 豁免。旧数据不回刷。
4. **阶段四 · 品种副轨 UI**：品种库、已点亮的品种格、普及度描边、点亮后的「品种不详」格。全库灰格呈现未收口（§7）。
5. **物种树**：§4.2 已落地。宠物条目挂到种级节点（不新开节点），种级俗名家犬/狼按来源选。
...
- **R3 品种格呈现（2026-09-09 新开）**：全库预印灰格、只亮自己拍到的——已否决。对猫狗也不行，更不许给马单独开例外。当前权宜：所有物种只显示已点亮的格子，对照表数据不按 `listed` 分等。这不是最终呈现，以后另想；**不要把灰格加回来，也不要按物种特殊处理。**
...
- [x] 首页「已收录 N」= 野生 + 宠物张数之和；宠物行仍在（§4.2）。
- [x] 已收录列表混排野生卡 / 宠物卡；点宠物卡进品种格。
- [x] 物种树挂宠物条目到种级节点；只狗标签「家犬」。
- [x] 宠物卡挂驯化卷稀有度 +「驯养物种」标签（R2）。
- 旧行不回刷（要重识别才进宠物图鉴）。

### 阶段四完成线（品种副轨 UI）

- [x] 品种库 `apps/api/data/breeds/`（cat / dog / chicken / cow / horse + discard；Wikidata 可重复合并）。
- [x] 品种格：猫狗马牛鸡同一路径；`listed` 不参与展示。全库预印灰格已否决，呈现方案见 §7（当前权宜只出已点亮）。普及度描边只作用在已点亮标准格。展示词过术语表。
- [x] 认证三刀：对上库 → 标准格；空/排除表 → 品种不详；其余非空 → 卡上自由格。狸花/橘猫/三花走排除表。格上文案「品种不详」。
...
| [CollectionSpeciesPage.tsx](../../apps/web/src/pages/CollectionSpeciesPage.tsx) / [CollectionPage.tsx](../../apps/web/src/pages/CollectionPage.tsx) | 已收录混排 | 4.2 | 野生卡+宠物卡两行；N=两表之和；点宠物进品种格 |
| [CollectionTreePage.tsx](../../apps/web/src/pages/CollectionTreePage.tsx) / [treeModel.ts](../../apps/web/src/tree/treeModel.ts) | 物种树挂宠物 | 4.2 | 同一种节点；只狗标签家犬；详情两张卡 |
| `apps/api/data/breeds/` | 品种库 | 四 | cat/dog/chicken/cow/horse.json + discard.json |
| [pets/breeds.ts](../../apps/api/src/pets/breeds.ts) | 品种纠偏 | 四 | `resolveBreed` 三刀；去量词；唯一包含 |
| [scripts/import-breed-wikidata.py](../../scripts/import-breed-wikidata.py) | 扩库 | 四 | Wikidata SPARQL 合并，保留已有 id |

> §4.2 已落地：`treeModel` 挂宠物条目 + 种级俗名；已收录混排宠物卡。套册按 R6 已点亮驯养，与树口径对齐。
