...
| 套册成就 | 引擎已通 | 配置驱动；正式内容二步另配（§7） |
| 后置 | 未做 | 旅途元数据、全量灌库、iOS/上架；套册策展 |
...
产品原则（见 [`docs/05` C.4](./05-技术方案.md)）：结算揭示；文案「当地引入/关注种」；国家级；无国家不警示；**不**折进稀有度。
...
- UI：图鉴页套册区（CSS 灰阶占位）
- 已配三本：`intertidal`（潮间带）、`urban_wild`（城市野趣）、`woodland_edge`（林缘遇见）；`fixture-pipeline` 默认关闭。
- 继续加册：只加 JSON + messages，不改引擎。

配置说明见 [`data/volumes/README.md`](../apps/api/data/volumes/README.md)。

## 8. 后置

- 套册：开包高潮接「点亮槽 / 整册转彩」反馈（API 已有字段）
- 套册二步：正式多本套册策展（内容 only）
...
- 表面：`--card`、`--card-muted`、`--input-bg`、`--surface-wash`、`--modal-bg`、`--modal-scrim`、`--nav-bg`、`--shadow`
