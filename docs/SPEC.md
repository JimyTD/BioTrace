...
**生效模型名写进缓存**——换模型后要靠它做离线复核，只存一个档位字母是查不出问题的。
...
**跨模型答案不完全一致是预期的**，不为换模型重新标定：实测同一只乌鸫，
`glm-5.1` 答「众多=是、喜爱=是」得 SSR，`glm-5.3` 答两个否得 SR。缓存留了模型名，需要时离线复核。
...
- [ ] 换模型后的离线复核：缓存已留 `model` / `score` / 12 题答案，复核工具与节奏未定
...
- [x] **不为换模型重新标定**：生产是模型链，逐档验收在成本与口径上都不成立（以谁为准？）。改为记录生效模型名，需要时离线复核。
