...
在开包 / 稀有度 / 图鉴之前分流「不是现场真实生物」的结果。2026-09-08 起改为**三档**：
...
- 三档统一标题回落链：`subjectTitle ?? commonName ?? scientificName ?? t("detail.unnamed")`。细节见 [`识图护栏.md`](./features/识图护栏.md) §5。

验收：真鸟可开包（相遇）；海报真牛 / 馆藏标本 → 名字+分类+「未相遇」+ 不进图鉴；书 / 卡通人 / 玩具熊 / 茶杯 → 相册留「留影」+ agent 短名 + 无分类，不用红字；过粗仍「未理想识别」但中性灰；断网/额度尽 → 才出红字。
...
- 品种库只纠偏，写在该次观察上，卡上不设品种排。空 / 排除词不展示品种；其余非空展示标准名或原文。
- 更新前已拍的照片不回刷。
