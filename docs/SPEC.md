...
- **软档（2026-09-07）**：真生物但非野外相遇——影像/印刷（海报、画布、屏幕、直播里的真牛）与馆藏标本（`specimen`）。有身份+有界 → `settled` + `identify_soft_encounter`：识别放行、全套字段保留（含保护级别标签），但**不进**结算/图鉴/套册/物种树；详情页「NC · 未收录」印章占稀有度徽章位（置首，视为一种稀有度档位），不另加状态徽章，下方理由块说明；相册格保留灰徽章「未相遇」。无身份或无界 → 照旧 `identify_not_living` 拦死。细节见 [`识图护栏.md`](./features/识图护栏.md) §5。
...
验收：书 / 卡通人 / 玩具熊 → 不可收集 + 梗句 + 无分类；真鸟可开包；过粗仍「未理想识别」；海报真牛 / 馆藏标本 → 名字+分类+「未相遇」+ 不进图鉴。
...
| 驯化身份注入 | 驯化卷不把驯化 / 室内 / 常空手交给模型；短窗口、分布窄仍问 |
...
| 驯化身份 | 驯化卷锁 `domesticated=true`、`indoor=false`、`often_absent=false`；室内扣分与常空手加分都不走。短窗口 / 分布窄仍问模型 | 圈舍、牧场、家里就是它的生境；被人养在屋里不是自住。旧 `|dom` 缓存本地回写，不升 `scale3` |
| 离人阶梯 | `indoor` 与 `near_home` 只扣最重的一级；港口 / 农田 / 郊野第三级不扣；驯化种不走 indoor | 都是「离人多近」，重复扣是双罚 |
  ...
  → computeSettle（settle/rules.ts）
      ...
      → resolveRarity（rarity/index.ts）
          → resolveScaleRarity（rarity/scale.ts）
              ...
              → 读缓存 scale2|有效国家|taxon（驯养加 `|dom`；命中后若身份三题与注入不一致则本地回写，不调模型）
              → 名录查表：灭绝 → XR，0 次模型调用
              → 3 批题走模型链（llm/text-chain.ts）；驯化卷摘掉驯化 / 室内 / 常空手 → 逐题取多数
