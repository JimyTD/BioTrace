...
2. 字段必须包含：
   ...
   - breed_zh：品种中文名或 null。仅 domesticated=true 时填写。能看出具体品种就写（英短、金渐层、荷斯坦，或你认出来的名字）。认不出、颜色系俗名（狸花/橘猫/三花）、串串/混血、只是物种名（家猫/家犬/家牛）：必须 null，不要写「品种不详」「家猫」当品种。俗名 common_name_zh 写物种（家犬/家猫），不要把品种写进俗名。
